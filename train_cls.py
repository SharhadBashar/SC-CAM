...
def get_size_path(img_name, size_dataset_path):
    tmp = os.path.join(dataset_path, img_name + '.pkl')
    return tmp
...
def getPenalty(label_size_n, size_n, C = 21):
    penalty_n = torch.zeros_like(label_size_n)
    for i in range(C):
        a, b = getAB(size_n[i])
        # a, b = [0.15, 1]
        if label_size_n[i] < a:
...
class Sub_Class_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        self.size_dataset_path = os.path.join(self.voc12_root, 'SegmentationClassAugSize')

        filename = self.filename[index]

        img = Image.open(get_img_path(filename, self.dataset_path)).convert('RGB') #filename = 2007_000032
        ...
        with open(get_size_path(filename, self.size_dataset_path), 'rb') as f:
            size = pickle.load(f)
        size = np.array(list(size.values()), dtype = np.float64)
        size = torch.from_numpy(size)

        return img, size, label_20, label_200, filename
...
if __name__ == '__main__':
    ...
    parent_labels = np.load('./voc1/cls_labels.npy', allow_pickle = True).tolist()
    k_cluster = 10
    ...
    for ep in range(args.max_epoches):
        ...
        for iter, (data, size, label_20, label_200, filename) in tqdm(enumerate(train_data_loader)):
            ...
            label_20_softmax = F.softmax(label_20)
            label_200_softmax = F.softmax(label_200)
            label_20_size = label_20_softmax.mean(3).mean(2)
            label_200_size = label_200_softmax.mean(3).mean(2)

            N_c, C = label_20.shape
            sum_N_c = 0
            sum_N_c_32x = 0

            for n in range(N_c):
                sum_C = torch.sum((label_20_size[n] - size[n].cuda()) ** 2)
                sum_C_32x = torch.sum((label_200_size[n] - size[n].cuda()) ** 2)

                sum_N_c += sum_C / (C - 1)
                sum_N_c_32x += sum_C_32x / (C - 1)

            loss_size = weight[5] * (sum_N_c + sum_N_c_32x) / N_c

            # #loss_2:
            # for n in range(N_c):
            #     pentaly_n = getPenalty(label_20_size[n], size[n])
            #     sum_C = torch.sum((label_20_size[n] - pentaly_n.cuda()) ** 2)

            #     pentaly32x_n = getPenalty(label_200_size[n], size[n])
            #     sum_C_32x = torch.sum((label_200_size[n] - pentaly32x_n.cuda()) ** 2)
            ...
            loss = cls_20_loss + (args.subcls_loss_weight * cls_200_loss) + loss_size

            if iter % 100 == 0:
                print('k{} R{}| Ep:{} L:{} -20_LOSS:{} -200_LOSS:{} SIZE_LOSS:{} | -cls20:{} | -cls200_Top1:{} -Sum:{}'.format(args.k_cluster, args.round_nb, ep, round(loss.item(), 3), round(cls_20_loss.item(), 3), round(cls_200_loss.item(), 3), round(loss_size.item(), 3), avg_cls20_ep_acc, avg_ep_p_acc, avg_ep_acc))
            ...
            writer.add_scalar('Size Loss', loss_size.item(), step)
